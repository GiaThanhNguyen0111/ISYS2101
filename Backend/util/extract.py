...
category = ['diary', 'meat', 'vegetable', 'fatty', 'grains']

def getOneElement(array):
...
def toJSON(recipe_id,recipe_name, ingredients, image_url, cooking_direction, nutritions, level, rating, meal_type, category):
    dict_object = {
        'id': recipe_id,
        'name': recipe_name,
...
def csv_to_JSON(filename):
    ...
    for index, record in lines.iterrows():
        ...
        recipe = toJSON(recipe_id,recipe_name, ingredients, image_url, cooking_directions, nutritions, random.randint(1,5), random.randint(1,5), getOneElement(meal_type), getOneElement(category))
        recipeList.append(recipe)

    return recipeList

def createIngredientList(filename):
    lines = readFile(filename)
    ingredientList = []
    for index, record in lines.iterrows():
        ingredients = extractedIngredients(record['ingredients'])
        for ingredient in ingredients:
            if ingredient not in ingredientList:
                ingredient = ingredientJSON(ingredient)
                ingredientList.append(ingredient)
    
    return ingredientList

def writeFileRecipe(filename):
...
def writeFileIngredients(filename):
    ingredientList = createIngredientList('D:\Extract files\core-data_recipe.csv')
    file = open(filename, 'a')

    for recipe in ingredientList:
        file.write(recipe+ ",\n")


writeFileRecipe('D:/Extract files/recipes.json')
# writeFileIngredients('D:/Extract files/ingredients.json')
